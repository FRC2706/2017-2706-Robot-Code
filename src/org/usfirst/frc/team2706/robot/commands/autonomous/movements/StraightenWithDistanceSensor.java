...
public class StraightenWithDistanceSensor extends Command {
	 ...
	 * 
	 */
	public StraightenWithDistanceSensor(double minSpeed, double maxSpeed, double baseSpeed, double error, double stopCycles, boolean inverted, double speedIncrease, double speedDecrease) {
	 ...
	 protected void execute() {
		 ...
		 double offset = Math.abs(Robot.driveTrain.getLeftDistanceToObstacle() - Robot.driveTrain.getRightDistanceToObstacle());
		 double turnSpeed = baseSpeed + (offset / speedIncrease) - (speedDecrease * currentStopCycles);
		 ...
		 if(Robot.driveTrain.getRightDistanceToObstacle() > Robot.driveTrain.getLeftDistanceToObstacle()) {
			 turnSpeed *= -1;
		 }
	...
	protected void inPosition() {
		if(Math.abs(Robot.driveTrain.getLeftDistanceToObstacle() - Robot.driveTrain.getDistanceToObstacle()) < error && 
				Math.abs(Robot.driveTrain.getRightDistanceToObstacle() - Robot.driveTrain.getDistanceToObstacle()) < error) {
			currentStopCycles++;
		}
		else {
			currentStopCycles--;
